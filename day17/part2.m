  ...
  . if (x=width)&(y=height) write "Part 2: ",+cost,! set finished=1
  . ; move up
  . if (y>1)&(up<10)&(down=0)&((left=0)!(left>3))&((right=0)!(right>3)) do enqueue(cost,x,y-1,up+1,0,0,0)
  . ; move down
  . if (y<height)&(down<10)&(up=0)&((left=0)!(left>3))&((right=0)!(right>3)) do enqueue(cost,x,y+1,0,0,down+1,0)
  . ; move left
  . if (x>1)&(left<10)&(right=0)&((up=0)!(up>3))&((down=0)!(down>3)) do enqueue(cost,x-1,y,0,0,0,left+1)
  . ; move right
  . if (x<width)&(right<10)&(left=0)&((up=0)!(up>3))&((down=0)!(down>3)) do enqueue(cost,x+1,y,0,right+1,0,0)
